...
old_defaults = models.Article.query.filter(
    models.Article.default_set == False).all()
...
for i in range(max_pages + 1)[1:]:
    try:
        response = urllib2.urlopen(
            'http://api.altmetric.com/v1/citations/1m?num_results=100&page={0}&key={1}'.format(i, ALTMETRIC_KEY))
